...
const WORKER_META: Record<string, { name: string; icon: string; color: string }> = {
...
function renderMarkdown(text: string): string {
  return marked.parse(preprocessContent(text), { breaks: true, gfm: true }) as string;
}
 ...
 */
function ThinkingProcess({ trace, defaultExpanded = false }: { trace: TraceStep[]; defaultExpanded?: boolean }) {
    ...
    <div className={`thinking-process ${expanded ? 'expanded' : 'collapsed'}`}>
      <button className="thinking-header" onClick={() => setExpanded(!expanded)}>
        ...
        <span className="thinking-summary">
          {allComplete ? (
            ...
            <span className="thinking-step">{currentWorker.step.replace(/^\d+\.\s*/, '')}</span>
          )}
        </span>
      ...
      {expanded && (
        <div className="thinking-body">
          {trace.map((step, i) => {
            const meta = WORKER_META[step.worker] || { name: step.worker, icon: '·', color: 'var(--ink-tertiary)' };
            return (
              <div key={i} className={`thinking-step-row ${step.status}`}>
                <div className="thinking-worker-icon" style={{ background: meta.color }}>
                  {meta.icon}
                </div>
                <div className="thinking-step-content">
                  <div className="thinking-step-title">
                    <span className="thinking-worker-name">{meta.name}</span>
                    <span className="thinking-step-name">{step.step.replace(/^\d+\.\s*/, '')}</span>
                    {step.duration_ms > 0 && (
                      <span className="thinking-duration-inline">{step.duration_ms}ms</span>
                    )}
                  </div>
                  ...
                  {step.status === 'running' ? (
                    ...
                    <div className="thinking-io">
                      <div className="thinking-io-row">
                        ...
                        <span className="thinking-io-text">{step.input_summary}</span>
                      </div>
                      <div className="thinking-io-row">
                        <span className="thinking-io-label">出</span>
                        <span className="thinking-io-text output">{step.output_summary}</span>
                      </div>
                      {step.action && (
                        <div className="thinking-io-row">
                          <span className="thinking-io-label">动</span>
                          <span className="thinking-io-text action">{step.action}</span>
                        </div>
                      )}
                    </div>
                  )}
...
export default function QAPage() {
  ...
  const simulateTrace = async (question: string): Promise<TraceStep[]> => {
    ...
    const createStep = (idx: number, step: string, worker: string, action: string, input: string, output: string): TraceStep => ({
      step: `${idx}. ${step}`,
      worker,
      action,
      input_summary: input,
    ...
    // 1. pi-user 接收问题
    steps.push(createStep(1, 'pi-user 接收问题', 'pi-user', 'understand_intent',
    ...
    // 2. pi-internal 规划
    steps.push(createStep(2, 'pi-internal 规划任务', 'pi-internal', 'plan_execution',
      '意图: 劳动纠纷 | 需求: 步骤化维权指南',
    ...
    // 3. knowledge 检索
    steps.push(createStep(3, 'knowledge 检索法条', 'knowledge', 'search_articles',
      '关键词: 工资 拖欠 维权 仲裁',
    ...
    // 4. ai-engine 准备生成
    steps.push(createStep(4, 'ai-engine 生成回答', 'ai-engine', 'synthesize',
  ...
  return (
    <div className="page qa-page">
          ...
          <div className="example-questions">
            ...
            <div className="example-grid">
              {EXAMPLE_QUESTIONS.map((q, i) => (
                <button key={i} className="example-btn" onClick={() => handleSubmit(q.text)}>
      ...
      <div className="qa-messages">
        {messages.map(msg => (
          <div key={msg.id} className={`message ${msg.role} phase-${msg.phase || 'done'}`}>
            ...
            <div className="message-content">
              ...
              ) : msg.role === 'assistant' && msg.streaming && msg.fullContent ? (
      ...
      <form className="qa-input" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
        <input
          type="text"
          value={input}
