...
export default function AnalysisPage() {
  ...
  const [analysisType, setAnalysisType] = useState("risk_review");
  ...
  const loadDocuments = async () => {
    try {
      const docs = await invoke<DocumentInfo[]>("list_documents", { limit: 100, offset: 0 });
      setDocuments(docs || []);
    } catch (e) {
  ...
  const handleAnalyze = async () => {
    ...
    setResult(null);
    try {
      const res = await invoke<AnalysisResult>("analyze_contract", {
          ...
          {result.risks.length > 0 && (
            <div className="risks-list">
              ...
              {result.risks.map((risk, index) => (
                <div key={index} className={`risk-item risk-${risk.risk_level}`}>
