...
import { docgen, type Template } from "../lib/api";
type TemplateInfo = Pick<Template, "id" | "name" | "category" | "description">;
...
export default function TemplatesPage() {
	...
	const loadTemplates = async () => {
		try {
			const tmpls = await docgen.listTemplates(category || undefined);
			setTemplates(
				(tmpls || []).map(({ id, name, category, description }) => ({
					id,
					name,
					category,
					description,
	...
	const handleSelectTemplate = async (id: string) => {
		try {
			const tmpl = await docgen.getTemplate(id);
			if (tmpl) {
	...
	const handleGenerate = async () => {
		...
		try {
			const result = await docgen.generate(selectedTemplate.id, variables);
		...
		<div className="page templates-page">
			...
			<div className="card">
				<div className="tabs">
					...
					<button
						className={`tab ${category === "contract" ? "active" : ""}`}
						onClick={() => setCategory("contract")}
					>
						合同
					</button>
					<button
						className={`tab ${category === "letter" ? "active" : ""}`}
						onClick={() => setCategory("letter")}
					>
						信函
					</button>
					<button
						className={`tab ${category === "report" ? "active" : ""}`}
